...
terraform_git \
  ...
  -backend-config="key=he_aws_tf_app_itf_abs_env_1_bug_001_fail.tfstate" \
  -input=false \
  -no-color \
  -upgrade \
...
terraform_git \
  workspace \
  new \
  he_aws_tf_app_itf_env_1_bug_001_fail \
  -no-color
```
...
terraform_git \
  ...
  -input=false \
  -no-color \
  -out="he_aws_tf_app_itf_abs_env_1_bug_001_fail.tfout" \
  -var="he_aws_tf_app_rgn_eu=[\"eu\", \"west\", \"2\", ]" \
  -var="he_aws_tf_app_rgn_us=[\"us\", \"east\", \"1\", ]" \
  -var-file="./he_aws_tf_mod_he_aws_tf_app_itf_cnf.tfvars"
```
